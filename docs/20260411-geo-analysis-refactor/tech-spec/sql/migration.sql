...
CREATE TABLE `geo_analysis_template` (
    ...
    `cron_expression`   VARCHAR(64)      DEFAULT NULL COMMENT 'Cron表达式（为空表示一次性任务）',
...
CREATE TABLE `geo_analysis_result` (
    ...
    `params`        TEXT             DEFAULT NULL COMMENT '关联任务信息JSON（batch任务ID+状态）',
    `result`        TEXT             DEFAULT NULL COMMENT '最终分析结果JSON',
