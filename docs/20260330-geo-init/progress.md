...
| API 服务改造 | ✅ 完成 | 移除 mock，连接真实后端 (hotword, content, dataCenter, publish) |
...
- **全部模块前端**（页面 + API 服务）
- **前端代理配置**（连接后端 8080）
- **测试用例文档**（按模块组织 + 85个E2E点击用例）
- **前端本地启动**（Node v12.16.1，端口 3000）
- **热词中心模块联调**（85个E2E测试用例全部通过）
...
3. 数据中心模块联调
4. 发布中心模块联调
5. QSchedule 定时任务（GEO 监控数据采集）

**关联需求：**
- 热词分析功能已拆分为独立需求 [docs/20260402-hotword-analysis/](../20260402-hotword-analysis/)，包含热词类型字段和热词分析任务功能
